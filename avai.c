...
void init(struct Node *header, struct Node *curr, char *s)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        if (strcmp(rules[i]->data, nonTerminal) == 0)
        {
            ...
            if (isTerminal(temp->data))
            {
                ...
                if (currNode == NULL)
                {
                    init(header, currNode, temp->data);
            ...
            {
                printf("HIIIIIII %s",(rules[i]->next)->data);
                //printf(" %d\n",isEpsilon(temp->data));
                nts[lhs].hasEpsilon = true;
            }
        }
    }
    return nts[get(nonTerminal)].first=header;
}
int main()
{
    printf("Hello world\n");
